...
library(snow)
#install.packages("qtl")
cross <- read.cross ("csv", "/users/limengxie/Desktop/JASHS_mqm//","Mapping181_11.csv",estimate.map=F)
...
cross.sc1.perms<-scanone(cross, pheno.col=1:(nphe(cross)-1), method="hk", n.perm=1000, verbose=TRUE, model="normal", n.cluster=4)
...
######################################
##Model Selection and QTL refinhkent##
######################################
...
for(k in 1:(nphe(cross)-1)){
  ...
  if(is.na(chr[1])==FALSE) {
    ...
    #(32) Contingency stathkent if no QTL are detected in the initial genome scan for a given phenotype. You can ignore this step if you are not running the entire model selection section as a loop. 
